...
## If true.lv is supplied, then create.life conditions on this and everthing in dist.mat is overwritten
...
create.life <- function(true.lv = NULL, lv.coefs, 
     lv.control = list(num.lv = 0, type = "independent"), #, lv.covparams = NULL, dist.mat = NULL
...
#                if(lv.control$type != "independent" & (is.null(lv.control$dist.mat) || is.null(lv.control$lv.covparams)))
#                     stop("For structured latent variables are to be generated, please supply both lv.control$dist.mat and lv.control$lv.covparams")
#                if(lv.control$type != "independent" & (nrow(dist.mat) != n || ncol(dist.mat) != n))
#                     stop("lv.control$dist.mat should be a symmetric matrix with the same number of rows as X")
#                
#                if(lv.control$lv.type == "exponential")
#                      covmat_chol <- (chol(exp(-lv.control$dist.mat/lv.control$lv.covparams[1])))
#                 if(lv.control$lv.type == "squared.exponential")
#                      covmat_chol <- (chol(exp(-(lv.control$dist.mat/lv.control$lv.covparams[1])^2)))
#                 if(lv.control$lv.type == "cauchy")
#                      covmat_chol <- (chol((1 + (lv.control$dist.mat/lv.control$lv.covparams[1])^2)^(-lv.control$lv.covparams[2])))
#                 if(lv.control$lv.type == "spherical")
#                      covmat_chol <- (chol((lv.control$dist.mat < lv.covparams[1])*(1 - 1.5*dist.mat/lv.control$lv.covparams[1] + 0.5*(dist.mat/lv.control$lv.covparams[1])^3)))
#                
...
#      
	...
	if(!save.params) 
		out <- sim_y
	if(save.params) 
		out <- list(resp = sim_y, true.lv = true.lv, lv.coefs = lv.coefs, X.coefs = X.coefs, traits.coefs = traits.coefs, row.params = row.params, row.coefs = row.coefs, cutoffs = cutoffs, powerparam = powerparam)
...
simulate.boral <- function(object, nsim = 1, seed = NULL, new.lvs = FALSE, est = "median", ...) { #, dist.mat = NULL
	...
	if(!is.null(seed)) 
      ...
      if(!new.lvs) {
          ...
          out <- replicate(nsim, create.life(true.lv = true_mod$lv, lv.coefs = true_mod$lv.coefs, X = object$X, X.coefs = true_mod$X.coefs, traits = object$traits, traits.coefs = true_mod$traits.coefs, family = object$family, row.eff = object$row.eff, row.params = true_mod$row.params, offset = object$offset, trial.size = object$trial.size, cutoffs = true_mod$cutoffs, powerparam = true_mod$powerparam))
          }
      if(new.lvs) {
          message("New latent variables generated for each simulated datasets")
          lv.control <- list(num.lv = object$num.lv, type = "independent")
#           lv.control <- object$lv.control
#           lv.control$lv.covparams <- true_mod$lv.covparams
#           if(lv.control$type != "independent" & is.null(dist.mat))
#                stop("For structured latent variables are to be generated, please supply both dist.mat")
#           lv.control$dist.mat <- dist.mat
#                
           out <- replicate(nsim, create.life(true.lv = NULL, lv.coefs = true_mod$lv.coefs, lv.control = lv.control, X = object$X, X.coefs = true_mod$X.coefs, traits = object$traits, traits.coefs = true_mod$traits.coefs, family = object$family, row.eff = object$row.eff, row.params = true_mod$row.params, offset = object$offset, trial.size = object$trial.size, cutoffs = true_mod$cutoffs, powerparam = true_mod$powerparam))
           }
