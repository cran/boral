...
check.lv.control <- function(num.lv, lv.control) {	
     if(is.null(lv.control$type))
          lv.control$type <- "independent"
     lv.control$type <- match.arg(lv.control$type, choices = c("independent","cauchy","exponential","squared.exponential","spherical"))

     if(!is.null(num.lv) & is.null(lv.control$num.lv)) {
          message("From boral version 1.5, num.lv is a redundant argument and replaced by lv.control$num.lv. Setting lv.control$num.lv = num.lv..")
          lv.control$num.lv <- num.lv
          }
          
     if(lv.control$type != "independent" & is.null(lv.control$dist.mat))
          stop("If structured latent variables are used, then please supply a distance matrix to lv.control$dist.mat")
...
process.geweke <- function(fit.mcmc, y, X = NULL, traits = NULL, family, num.lv, 
     row.eff, row.ids, num.ord.levels = NULL, prior.control) { #lv.type = "independent"
          p <- ncol(y)
          num.X <- 0
          if(!is.null(X)) 
			num.X <- ncol(X)
