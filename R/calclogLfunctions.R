...
# 			for(i in 1:n) { loglik_comp[i,j] <- log(get_probs[as.vector(y[i,j])]) } }
	...
	if(row.eff == "random") {
		...
		for(i in 1:n) {
			spp_f <- eta <- matrix(lv.coefs[, 1], nrow = mc.row.eff, ncol = p, byrow = TRUE)
               for(k in 1:ncol(row.ids)) 
			...
			if(!is.null(offset)) 
				...
				for(j in 1:p) {
					if(complete_family[j] == "binomial") 
                              spp_f[, j] <- dbinom(rep(as.vector(y[i, j]), mc.row.eff), complete_trial_size[j], prob = pnorm(eta[, j]))
					if(complete_family[j] == "poisson") 
                              spp_f[, j] <- dpois(rep(as.vector(y[i, j]), mc.row.eff), exp(eta[, j]))
					if(complete_family[j] == "negative.binomial") 
                              spp_f[, j] <- dnbinom(rep(as.vector(y[i, j]), mc.row.eff), size =1/(lv.coefs[j, ncol(lv.coefs)]+1e-5), mu = exp(eta[, j]))
					if(complete_family[j] == "exponential") 
                              spp_f[, j] <- dexp(rep(as.vector(y[i, j]), mc.row.eff), rate = 1/exp(eta[, j]))
					if(complete_family[j] == "gamma") 
                              spp_f[, j] <- dgamma(rep(as.vector(y[i, j]), mc.row.eff), shape = exp(eta[, j]) * lv.coefs[j, ncol(lv.coefs)], rate = lv.coefs[j, ncol(lv.coefs)])
					if(complete_family[j] == "beta") 
                              spp_f[, j] <- dbeta(rep(as.vector(y[i, j]), mc.row.eff), lv.coefs[j, ncol(lv.coefs)] * exp(eta[, j])/(1 + exp(eta[, j])), lv.coefs[j, ncol(lv.coefs)] * (1 - exp(eta[, j])/(1 + exp(eta[, j]))))
					if(complete_family[j] == "normal") 
                              spp_f[, j] <- dnorm(rep(as.vector(y[i, j]), mc.row.eff), mean = eta[, j], sd = (lv.coefs[j, ncol(lv.coefs)]))
					if(complete_family[j] == "lnormal") 
                              spp_f[, j] <- dlnorm(rep(as.vector(y[i, j]), mc.row.eff), meanlog = eta[, j], sdlog = (lv.coefs[j,ncol(lv.coefs)]))
					if(complete_family[j] == "tweedie") { 
						spp_f[, j] <- dTweedie(rep(as.vector(y[i, j]), mc.row.eff), mu = exp(eta[, j]), phi = lv.coefs[j, ncol(lv.coefs)] + 1e-06, p = powerparam, LOG = FALSE)
						spp_f[, j][which(spp_f[, j] == 0)] <- 1 
						}
					if(complete_family[j] == "ordinal") {
						get_probs <- ordinal.conversion.special(lv.coefs.j = lv.coefs[j, ], row.coefs.i = rnorm(mc.row.eff, 0, sd = row.params[[1]]), X.i = X[i, ], X.coefs.j = X.coefs[j, ], cutoffs = cutoffs)
						spp_f[, j] <- get_probs[, as.vector(y[i, j])] + 1e-05 
						}
					}
				
				spp_f[!is.finite(spp_f)] = 1
				spp_f <- matrix(spp_f, nrow = mc.row.eff, byrow = FALSE)
...
calc.marglogLik <- function (y, X = NULL, family, trial.size = 1, lv.coefs, 
	...
	index_multinom_cols <- which(complete_family == "multinom")
     if(row.eff == "random") {
          mc.row.coefs <- vector("list", ncol(row.ids))
          for(k in 1:ncol(row.ids)) 
               mc.row.coefs[[k]] <- matrix(rnorm(length(unique(row.ids[,k]))*numrows_mclv, mean = 0, sd = row.params[[k]]), nrow = numrows_mclv) 
          }
